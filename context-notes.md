...
## 제어 비프 2번 이슈 (2026-06-29)
- 증상: HA에서 명령 1번 → 실내기 비프 2번.
- 진단 한계: EW11는 자기 RS485 TX를 다른 TCP 클라이언트로 echo 안 함 → 스니퍼로 우리 명령(80.ff.00) 관측 불가(`control_trace.py`에서 0개). HA가 1번/2번 보내는지 버스로는 못 봄.
- 데이터로 확정한 것: 실내기 20.00.01 Notification은 균일 간격이 아니라 **수 초 버스트**(8초 공백 관측). 제 재전송은 1초 간격이라 ack 전에 재전송 발동 → 비프 2번. 가장 유력.
- 조치: **재전송 기본 OFF** (`SEND_MAX_RETRIES=0`). 제어는 첫 전송으로 이미 동작. `send_request`에 TX debug 로그 추가.
- 검증 방법: 업데이트 후 비프가 1번이면 원인=재전송(해결). 여전히 2번이면 → HA debug 로그(`custom_components.samsung_nasa`)에서 "TX command" 줄이 1개인지 확인. 1개인데 2번 울면 실내기 하드웨어가 원래 2번 우는 것(코드로 못 고침).
