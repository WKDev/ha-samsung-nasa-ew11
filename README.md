...
- **명령당 비프 1회는 정상입니다.** 삼성 실내기는 리모컨이든 우리 버스 컨트롤러든
  **명령을 받으면 하드웨어가 확인음을 냅니다.** 프로토콜로 끌 수 없습니다
  (관련 미해결 이슈: esphome_samsung_hvac_bus#144, silence 코드 0x4045/0x4046 무효).
- **재전송은 기본 OFF**입니다. 실내기는 컨트롤러에 `Ack`를 안 보내고 상태 `Notification`만
  브로드캐스트하는데, 그 Notification이 수 초 단위 버스트로 와서 blind 재전송을 켜두면
  ack 전에 재전송이 터져 **비프가 2번** 납니다. EW11 경유 송신은 첫 전송으로 안정적으로
  도달하므로 한 번만 보냅니다. (재전송이 필요하면 `const.py`의 `SEND_MAX_RETRIES`를
  올리세요. 그러면 `notify-as-ack`로 응답 수신 시 재전송을 멈춥니다.)
- **온도 변경만 1~2초 지연되는 건 정상**입니다. 이건 버스가 아니라 **Home Assistant
  온도조절 카드의 디바운스** 때문입니다(조정을 멈출 때까지 기다렸다가 한 번 전송).
  전원·풍량 같은 이산 명령은 즉시 나갑니다. 즉시 보내려면 `climate.set_temperature`
  서비스를 직접 호출하면 됩니다.
